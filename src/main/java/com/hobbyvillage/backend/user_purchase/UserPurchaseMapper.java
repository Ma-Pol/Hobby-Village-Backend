...
public interface UserPurchaseMapper {
	...
	// (추가 주문 x) 결제 성공 시 장바구니 상품 데이터 삭제
	@Delete("DELETE FROM carts WHERE email = #{email} AND prodCode = #{prodCode};")
	void deleteCart(@Param("email") String email, @Param("prodCode") String prodCode);
